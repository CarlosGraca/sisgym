...
public class ModeloTabelaEquipamento extends AbstractTableModel {
    ...
    private ArrayList<String[]> resultSetEquipamento;
    private ResultSetMetaData rsMetaData;
    private static final String[] nomesColunas = {
        "Codigo", "Nome","Marca","CodEquipamento"
    };
    public String getColumnName(int quantasColunas) {
        return nomesColunas[quantasColunas];
    }

    public ModeloTabelaEquipamento(ResultSet resultset) throws SQLException {
        setResult(resultset);
    }
    
    public void setResult(ResultSet resultset) throws SQLException {
        resultSetEquipamento = new ArrayList<String[]>();
        resultset.beforeFirst();
        while (resultset.next()) {
            String[] linha = {
                resultset.getString("idEquipamento"),
                resultset.getString("nome"),
                resultset.getString("marca"),
                resultset.getString("codEquipamento"),
            };
            resultSetEquipamento.add(linha);
        }
        fireTableStructureChanged();
    }
    public void deletarLinha(int linha) {
        resultSetEquipamento.remove(linha);
    ...
    public int getRowCount() {
        return resultSetEquipamento.size();
    }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        String[] linha = resultSetEquipamento.get(rowIndex);
